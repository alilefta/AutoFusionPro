...
{
    public partial class VehicleTaxonomyService : IVehicleTaxonomyService
    {

        #region Fields

        private readonly IUnitOfWork _unitOfWork;
        private readonly ILogger<VehicleTaxonomyService> _logger;
        ...
        public VehicleTaxonomyService(
                    IUnitOfWork unitOfWork,
                    ILogger<VehicleTaxonomyService> logger,
       ...
       // public async Task<CompatibleVehicleSummaryDto> GetCompatibleVehicleSummaryByIdAsync(int id)
       // {
       //     if (id <= 0)
       //     {
       //         _logger.LogWarning("Attempted to get CompatibleVehicle with invalid ID: {CompatibleVehicleId}", id);
       ...
       //         }
       //         _logger.LogInformation("Successfully retrieved CompatibleVehicle with ID {CompatibleVehicleId}", id);
       //         return MapCompatibleVehicleToSummaryDto(compatibleVehicle);
       //     }
       //     catch (Exception ex)
       //     {
       //         _logger.LogError(ex, "An error occurred while retrieving CompatibleVehicle with ID {CompatibleVehicleId}.", id);
       ...
       // #endregion
        ...
        public async Task DeleteMakeAsync(int id)
        {
            ...
            if (hasModels)
            {
               dependentEntities.Add(nameof(Model));
            }

            bool hasAssociatedRule = await _unitOfWork.Makes.IsUsedInCompatibilityRulesAsync(id); // Example
            if (hasAssociatedRule)
            {
                dependentEntities.Add(nameof(PartCompatibilityRule));
            }

            if (dependentEntities.Any())
            {
                string errorMessage = $"Cannot delete Make '{makeToDelete.Name}' (ID: {id}) as it has associations.";
                _logger.LogError(errorMessage + $" Dependent types: {string.Join(", ", dependentEntities)}");
                throw new DeletionBlockedException(
                    errorMessage,
                    nameof(Model),
                    makeToDelete.Id.ToString(),
                    dependentEntities
                );
            }
        ...
        public async Task DeleteModelAsync(int id)
        {
            ...
            bool hasAssociatedRule = await _unitOfWork.Models.IsUsedInCompatibilityRulesAsync(id); // Assumes this method exists
            if (hasAssociatedRule)
            {
                dependentEntities.Add(nameof(PartCompatibilityRule));
            }
        ...
        public async Task DeleteTransmissionTypeAsync(int id)
        {
            ...
            bool hasAssociatedRule = await _unitOfWork.TransmissionTypes.IsUsedInCompatibilityRuleAttributesAsync(id);
            if (hasAssociatedRule)
            {
                ...
                throw new DeletionBlockedException(depError, nameof(TransmissionType), transmissionTypeToDelete.Id.ToString(), new List<string> { nameof(PartCompatibilityRuleTransmissionType) });
            }
        ...
        public async Task DeleteEngineTypeAsync(int id)
        {
            ...
            bool hasAssociatedRule = await _unitOfWork.EngineTypes.IsUsedInCompatibilityRuleAttributesAsync(id);
            if (hasAssociatedRule)
            {
                ...
                _logger.LogError(depError);
                throw new DeletionBlockedException(depError, nameof(EngineType), engineTypeToDelete.Id.ToString(), new List<string> { nameof(PartCompatibilityRuleEngineType) });
            }
        ...
        public async Task DeleteTrimLevelAsync(int id)
        {
            ...
            // Dependency Check: CompatibleVehicles
            bool hasAssociatedRule = await _unitOfWork.TrimLevels.IsUsedInCompatibilityRuleAttributesAsync(id);
            if (hasAssociatedRule)
            {
                string depError = $"Cannot delete Trim Level '{trimLevelToDelete.Name}' (ID: {id}) as it is used in Compatible Vehicle specifications.";
                _logger.LogError(depError);
                throw new DeletionBlockedException(depError, nameof(TrimLevel), trimLevelToDelete.Id.ToString(), new List<string> { nameof(PartCompatibilityRuleTrimLevel) });
            }
        ...
        public async Task DeleteBodyTypeAsync(int id)
        {
            ...
            bool hasAssociatedRule = await _unitOfWork.BodyTypes.IsUsedInCompatibilityRuleAttributesAsync(id);
            if (hasAssociatedRule)
            {
                string depError = $"Cannot delete Body Type '{bodyTypeToDelete.Name}' (ID: {id}) as it is used in Compatible Vehicle specifications.";
                _logger.LogError(depError);
                throw new DeletionBlockedException(depError, nameof(BodyType), bodyTypeToDelete.Id.ToString(), new List<string> { nameof(PartCompatibilityRuleBodyType) });
            }
